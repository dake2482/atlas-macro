...
_HHDC_DATABASE_QUANTUM = Decimal("0.00000001")
_HHDC_DELINQUENCY_NUMBER_FORMAT = "0.00"
_HHDC_HISTORY_START = date(2003, 3, 31)
...
def _hhdc_decimal(
    cell: Any,
    *,
    use_display_precision: bool,
) -> tuple[Decimal | None, dict[str, Any]]:
    """Normalize one HHDC XLSX number under the source worksheet contract.

    Excel calculates with at most 15 significant decimal digits, while OOXML
    formula caches and openpyxl expose the underlying binary-float tail.  The
    HHDC balance table publishes its underlying amounts, so stripping that tail
    recovers the source value without changing its decimal granularity.  The
    delinquency table instead declares a fixed-decimal display format; those
    ratios are normalized to that declared precision and retain the pre-rounded
    value and rule in row metadata.  The caller still rejects anything that is
    not exactly representable by ``Observation.value``.
    """

    raw_value = cell.value
    if raw_value is None or (
        isinstance(raw_value, str) and raw_value in {"", ".", "n.a."}
    ):
        return None, {}
    if (
        cell.data_type != "n"
    ...
    if source_value < 0:
        raise ValueError("NY Fed balance or delinquency value is negative")
    if use_display_precision and source_value > 100:
        raise ValueError("NY Fed delinquency value exceeds 100 percent")

    number_format = str(cell.number_format or "General")
    normalized = source_value
    normalization_rule = "excel-15-significant-digits"
    metadata: dict[str, Any] = {
        "parsed_xlsx_numeric_value": str(raw_value),
        "source_numeric_value": str(source_value),
        "source_number_format": number_format,
    }
    if use_display_precision:
        if number_format != _HHDC_DELINQUENCY_NUMBER_FORMAT:
            raise ValueError(
                "NY Fed delinquency value lacks the required 0.00 source format"
            )
        decimal_places = 2
        quantum = Decimal(1).scaleb(-decimal_places)
        normalized = source_value.quantize(quantum, rounding=ROUND_HALF_UP)
        normalization_rule = "xlsx-fixed-decimal-round-half-up"
        metadata["source_decimal_places"] = decimal_places

    try:
        database_value = normalized.quantize(_HHDC_DATABASE_QUANTUM)
    except (ArithmeticError, ValueError) as exc:
        raise ValueError("NY Fed value exceeds normalized database precision") from exc
    if normalized != database_value:
        raise ValueError("NY Fed value exceeds normalized database precision")
    metadata["normalization_rule"] = normalization_rule
...
class NYFedHouseholdDebtProvider(HTTPProvider):
    ...
    def _parse_workbook(payload: bytes) -> tuple[list[dict[str, Any]], str, str]:
        ...
        try:
            ...
            balance_records, balance_latest, balance_periods = _parse_hhdc_sheet(
                workbook["Page 3 Data"],
                HHDC_BALANCE_SERIES,
                unit="USD trillions",
                chart_name="Total Debt Balance and Its Composition",
                use_display_precision=False,
                total_components=HHDC_BALANCE_COMPONENTS,
            )
            delinquency_records, delinquency_latest, delinquency_periods = _parse_hhdc_sheet(
            ...
            workbook.close()
        if balance_periods != delinquency_periods:
            raise ValueError(
                "NY Fed balance and delinquency sheets have different quarterly coverage"
            )
...
def _quarter_label(value: Any) -> tuple[str, str] | None:
    if not isinstance(value, str):
        return None
    match = re.fullmatch(r"(\d{2}|\d{4}):Q([1-4])", value)
    if match is None:
        return None
...
def _parse_hhdc_sheet(
    ...
    *,
    ...
    for row in sheet.iter_rows(min_row=5):
        raw_period = row[0].value if row else None
        period = _quarter_label(raw_period)
        if period is None:
            if any(
                index < len(row)
                and row[index].value is not None
                and not (
                    isinstance(row[index].value, str)
                    and row[index].value in {"", ".", "n.a."}
                )
                for index in positions.values()
            ):
                raise ValueError(f"NY Fed {chart_name} quarterly period is invalid")
            continue
    ...
    if not ordered_periods or ordered_periods[0] != _HHDC_HISTORY_START:
        raise ValueError(
            f"NY Fed {chart_name} history must start at 2003:Q1"
        )
    for previous, current in zip(ordered_periods, ordered_periods[1:], strict=False):
    ...
    for period in ordered_periods:
        ...
        for name, series_id in series_map.items():
            ...
            value, precision_metadata = (
                _hhdc_decimal(
                    cell,
                    use_display_precision=use_display_precision,
                )
                if cell is not None
                else (None, {})
            )
            if value is None:
                ...
                raise ValueError(
                    f"NY Fed {chart_name} required value is missing for "
                    f"{name} at {period_label}"
                )
            period_values[name] = value
            latest_by_series[series_id] = value_date
        ...
        if total_components:
            component_total = sum(
                (period_values[name] for name in total_components),
                start=Decimal("0"),
            )
            if period_values.get("Total") != component_total:
                raise ValueError(
                    f"NY Fed {chart_name} Total does not equal its six components "
                    f"at {period_label}"
                )
        records.extend(period_records)
    if not records or set(latest_by_series) != set(series_map.values()):
        raise ValueError(
            f"NY Fed {chart_name} did not provide complete required history"
        )
    latest_periods = set(latest_by_series.values())
    if len(latest_periods) != 1:
    ...
    return (
        records,
        latest_periods.pop(),
        frozenset(period.isoformat() for period in ordered_periods),
    )
