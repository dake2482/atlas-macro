...
dependencies:
- AI-RESEARCH-014
updated_at: '2026-07-14T11:47:18+08:00'
next_action: Run a clean temporary-database official-source refresh and 1440/390 browser acceptance. Production deployment and production data refresh remain separately authorized actions.
evidence:
...
- Nine deterministic rate-spread tests pass, including Treasury field drift, exact-date publication, latest-failure retention, legacy weekly compatibility, missing-component empty state, licence revocation, and stale-component presentation.
- Repository gates pass: full Ruff, complete pytest, Django check, migration drift check, and git diff whitespace validation.
...
- [ ] 日频组件失败、日期回退、重复触发、批次混装、许可撤销、报价口径漂移、样本不足及发布后置条件失败均保留上一完整组件并显示 stale。
